function getAbsoluteUrl() {
    ...
    if (returnUrl.indexOf("/teams/") === -1 && returnUrl.indexOf("/sites/") === -1) {
        ...
        //console.log("non teams/sites url found");
    }
    ...
    if (returnUrl.indexOf("/sites/") !== -1) {
        ...
        //console.log("sites url found");
    }

    //console.log("Absolute URL: " + returnUrl);
    return returnUrl;
...
function AddDevToolsButton(id, btn_title, img_x, img_y, callback) {
    if (id == "openClosePanel" && document.getElementById("openClosePanel") !== null) {
        //console.log("[SPDL] Panel already added.");
        return;
    }
    ...
    if (id == "openClosePanel" && typeof _spPageContextInfo != "undefined" && _spPageContextInfo.webUIVersion === 4) {
        ...
        //console.log("[SPDL] SP2010 detected");
    }
    ...
    if (id == "openClosePanel" && document.getElementById("O365_TopMenu") !== null && window.location.href.indexOf(".sharepoint.com") === -1) {
        ...
        //console.log("[SPDL] SP2016/2019 detected");
    }
    ...
    if (id == "openClosePanel" && document.getElementById("O365_TopMenu") === null && typeof _spPageContextInfo != "undefined" && _spPageContextInfo.webUIVersion === 15 && window.location.href.indexOf(".sharepoint.com") === -1) {
        ...
        //console.log("[SPDL] SP2013 detected");
    }
    ...
    if (id == "openClosePanel" && document.getElementById("O365_TopMenu") === null && window.location.href.indexOf(".sharepoint.com") !== -1 && typeof _spPageContextInfo != "undefined") {
        ...
        //console.log("[SPDL] SPO detected (context available)");
        ...
        if(typeof unsafeWindow != "undefined")
        {
            ...
            console.log("[SPDL] _spPageContextInfo instantiated");
        }
        */
    }
    ...
    if (id == "openClosePanel" && document.getElementById("O365_TopMenu") === null && window.location.href.indexOf(".sharepoint.com") !== -1 && typeof _spPageContextInfo == "undefined" && $("#O365_MainLink_Settings_container").children().length === 0) {
        rootElement = '#HeaderButtonRegion';
        //console.log("[SPDL] SPO detected (context not available) [FAST]");
    }
    if (id == "openClosePanel" && document.getElementById("O365_TopMenu") === null && window.location.href.indexOf(".sharepoint.com") !== -1 && typeof _spPageContextInfo == "undefined" && $("#O365_MainLink_Settings_container").children().length !== 0) {
        rootElement = '#HeaderButtonRegion';
        //console.log("[SPDL] SPO detected (context not available) [SLOW]");
    }
...
function GoToListSettings() {
    $('#listSettingsLink').click(function () {
        if (typeof _spPageContextInfo != "undefined" && _spPageContextInfo.pageListId !== null) {
            var goToSettingsListUrl = getAbsoluteUrl() + "/_layouts/15/listedit.aspx?List=" + _spPageContextInfo.pageListId;
            console.log("list url: " + goToSettingsListUrl);
...
$(document).ready(function () {
    ...
    var panelHtml = '<div id="spdevtools-panel-wrapper" style="overflow:auto; top: 83px;right: 0;background: lightgray;width: 250px;height: 100%;z-index: 1000000;padding: 20px;"><div id="spdevtools-panel" style=""></div></div>';
    $("body").append(panelHtml);

    /*
    var mouseScriptHtml = `
    <script type='text/javascript' id='mouseScript'>
    if(window.location.href.indexOf('.sharepoint.com') !== -1)
    {
    ...
    `;
    */
    //$("body").append(mouseScriptHtml);

    //CONTEXT EXISTS: DEFINITELY A SHAREPOINT SITE
    //if (typeof _spPageContextInfo !== 'undefined' && window.location.href.indexOf(".sharepoint.com") === -1) {
    
        //CONTEXT DOES NOT EXIST - sharepoint.com --> add button (quickly) + readd button if DOM is reset
        //console.log("[SPDL] _spPageContextInfo not defined!");
        if (true) {
            //if(document.getElementsByTagName("html")[0].cloneNode(false).outerHTML.indexOf("ms-dialog") !== -1)
            if (window.self !== window.top) {
                //console.log("[SPDL] Dialog");
                return;
            }
            ...
            var checkExist = setInterval(function () {
                ...
                if (timesRun === 100) {
                    ...
                    //console.log("[SPDL] SP not found");
                }
                else {
                    //console.log("[SPDL] Waiting for DOM");
                    ...
                    if (document.getElementById("HeaderButtonRegion") !== null && fastAdded === false) {
                        //console.log("[SPDL] Adding button (first time)");
                    ...
                    if (document.getElementById("HeaderButtonRegion") !== null && $("#O365_MainLink_Settings_container").children().length !== 0) {
                        //console.log("[SPDL] Button added at later time");
                        AddDevToolsButton('openClosePanel', '', '-128px', '-32px', openClosePanel);
                        clearInterval(checkExist);
                    }
                }
    ...
    AddDevToolsButton('listPreservHold', 'Go to preserv hold', '-152px', '0', GoToPreservHold);//-169px -56px
